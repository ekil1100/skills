---
name: commit-push
description: 当用户要求 commit、push、提交、推送、生成 commit message，或说“commit msg 语义化”“帮我提交并推送”时使用此 skill。它会检查 git diff，生成 Conventional Commits 消息，并按需提交或推送。
---

# Commit Push

用于从查看改动到生成语义化提交、提交并按需推送的 Git 工作流。目标是提交内容准确、范围克制，并避免带入无关文件或敏感内容。
...
6. 生成 Conventional Commits 消息并提交。
...
- `scope` 优先用受影响模块、目录、功能名或 skill 名；没有清晰范围时省略。
- `subject` 简短描述行为变化，不加句号，避免 `update files`、`fix bug`、`misc changes`。
- 英文 subject 用祈使句、小写开头；中文 subject 保持简洁明确。
- 只有改动原因、迁移影响、破坏性变化或重要取舍需要说明时才加 body。
- 破坏性变化用 `BREAKING CHANGE:`；关联 issue 可用 `Refs #123` 或 `Closes #123`。

示例：

```text
docs(commit-push): 精简提交流程说明
