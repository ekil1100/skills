...
按主文档的[运行历史与输出目录](../SKILL.md#运行历史与输出目录)准备外层持久记录，检查**本次实际执行端的工具**。本节用于了解实际路径和判定依据，不是输出能力门禁；外部脚本可沿用默认目录和日志行为，不要求新增输出接口。

| 入口 | 核对位置 | 需要确认的能力 |
|---|---|---|
| 内部 JS/TS | 当前 `ecmascript/arksteed/test/README.md`、runner 的 CLI 和产物目录创建/写入代码 | abc、IR/反汇编、stdout/stderr、元数据和汇总各自的实际路径；可用输出配置及覆盖/清理行为。硬编码 `/tmp` 不阻塞执行。 |
| external | 当前 external checkout 的 README、runner CLI 和产物写入代码 | 独立核对输出能力，不套用内部 runner 的参数；`-R` 用于定位构建树，不等同于结果目录。 |
| 原生 unittest | 当前 `ecmascript/arksteed/unittests/BUILD.gn` → `test/test_helper.gni` 的 action → 实际执行脚本 | 本轮测试是否实际执行、覆盖哪些目标，失败/超时是否正确传递到入口退出码；有哪些摘要、日志或 ZIP 结果可用。成功用例日志不完整时注明，不作为执行门禁。 |
| 构建 | 当前 `ark.py` 及其日志调用链 | 完整构建控制台能否由记录器保存；工具在 `out` 中保留附带副本不构成阻塞。 |

按检查结果处理：

1. **支持输出配置**：可使用已核实的接口配置本轮目录，也可沿用工具默认目录；记录实际路径。
2. **路径固定或日志不完整**：照常执行请求的测试，保存外层可见输出、摘要和退出码；已有文件可按需复制归档，注明原路径及覆盖/清理风险。缺失日志如实记录，不标为 `BLOCKED`。
3. **判定依据不足**：目录或日志限制不阻止执行，但若无法确认本轮实际执行、请求范围或失败传播，结果标为未验证，不能只凭入口返回 0 宣称通过。仅实际环境/执行障碍记为 `BLOCKED`。

记录器能保存控制台并提供 `ARK_RESULTS_DIR`、`TMPDIR`，不能改变工具硬编码的路径，也不能恢复 action 未透出的输出。只使用真实支持的 CLI/GN 参数，不为统一输出改造外部脚本或修改全局软链接。

目录存在或 `ninja: no work to do` 均不是本轮测试通过的证据；核对本轮实际执行、可用结果和退出码传播，不把增量缓存当作新验证。具体接口、源码版本和证据缺口记在当次报告，不绑定本指南到本地补丁。

支持输出配置时可在 `$ARK_RESULTS_DIR` 下按配置及 `internal/`、`external/`、`unittest/` 分目录；否则记录工具实际路径。外层配置汇总写 `$ARK_RESULTS_DIR/config-summary.tsv`，与工具原生元数据、汇总分开保存。
...
了解输出接口后，采用 **配置间串行、配置内先构建并完成 unittest，再并行运行内部与 external** 的编排：
...
3. unittest 成功后，两套 JS/TS runner 都显式加 `-F`，测试期间只读共享构建产物。确认两套 runner 的实际结果路径不会互相覆盖后并行启动，无法隔离时改为套件串行。不要将两个默认带构建的 runner 直接放到后台并行。
...
以下列出 x64 Debug 的三个测试入口及选择参数，外部脚本默认沿用自身输出目录；有已核实的输出配置时可按需追加。将所选命令交给记录器执行，并按下述规则编排和收集退出码。只按用户授权替换平台/模式，本 skill 不规定主机连接或源码部署方式。
...
- unittest 成功后，以 `-F -j "$ARKSTEED_INTERNAL_JOBS"` 和 `-F -j "$ARKSTEED_EXTERNAL_JOBS"` 并行两套 runner；external 另带 `-R "$ark_root"`。各自 stdout/stderr 从启动起重定向到本轮 `$ARK_RUN_DIR/logs/` 下不同的配置/套件文件，原始产物沿用 runner 实际路径，按需复制到 `results/` 归档并记录来源。全量不加 `-s`。
...
- 记录源码/依赖版本、完整 external commit、命令、worker 数、收集/执行/通过/失败/超时/未执行数量及实际日志路径；GTest 数量按可用输出或结果文件统计，无依据时标为未知，不用目标数代替用例数。
...
正常结束或因配置失败/未验证而停止后，立即根据配置级 `config-summary.tsv`、unittest 可用的 GTest/GN action 输出、两套 runner 的汇总、元数据和日志输出中文报告，不等待或补跑后续配置。文件缺失或成功日志不完整时注明证据限制，不单独视为测试失败或阻塞。报告包含：

1. 按约定顺序列出用户要求的全部配置，标明通过、失败、未验证或未执行；未验证项注明证据缺口，未执行项注明前序配置失败/未验证及触发配置，或实际环境阻塞原因。
...
4. 附源码与依赖版本、实际命令、worker 分配依据、配置汇总和三套原始报告或日志的实际路径，注明实际执行主机，并提供运行 ID 与持久记录目录。区分原始路径、归档副本和缺失内容；临时目录可能被清理，不能称为不可变历史。已有 `.agents/` 累计报告链接外层记录和可用归档。将复跑或修复建议作为后续事项，不自动继续未执行配置。

明确区分通过、失败、未验证和未执行/受阻。环境不可用、缺少 external 或测试失败不能改判为通过或不涉及；前序配置通过不代表整套矩阵通过，只有约定范围内的全部用例执行且通过，才报告本次全量功能测试通过。
